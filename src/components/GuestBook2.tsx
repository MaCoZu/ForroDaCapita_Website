...
renderer.list = (token: Tokens.List) => {
  ...
  const startAttr = token.ordered && token.start !== 1 && token.start !== undefined ? ` start="${token.start}"` : ""
  const listClass = token.ordered ? "list-decimal list-inside space-y-1 ml-4" : "list-disc list-inside space-y-1 ml-4"
...
renderer.link = ({ href = "", title, text = "" }: LinkRendererParams) => {
  ...
  const attributes = {
    href: sanitizedHref,
    class: "text-accent hover:text-accent/80 underline transition-colors break-all",
    target: "_blank",
...
renderer.heading = ({
  ...
  const className = headingClasses[depth as keyof typeof headingClasses] || headingClasses[6]
...
const MessageBoard: FC<MessageBoardProps> = ({ containerClasses }) => {
  ...
  const insertMarkdown = (before: string, after = "", placeholder = "") => {
    ...
    if (selectedText) {
      ...
      newText = message.slice(0, start) + before + selectedText + after + message.slice(end)
      ...
      newText = message.slice(0, start) + before + textToInsert + after + message.slice(end)
  ...
  const renderMessages = () => {
    ...
    if (messages.length === 0) {
      return <p className="text-gray-500 text-center">No messages yet. Be the first!</p>
    }

    return (
      <div className="space-y-4">
        {messages.map((msg) => (
          <div key={msg.id} className="text-base-content bg-base-100 border-base-content/20 border-b pb-4 text-lg">
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(
                  ...
                  }) as string,
                  {
                    ...
                    ALLOWED_ATTR: ["href", "title", "class", "target", "rel", "src", "alt", "loading"],
                    ALLOWED_TAGS: [
                      "p",
                      "br",
                      "strong",
            ...
            />
            <small className="text-gray-500 block mt-2">{formatDate(msg.created_at)}</small>
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className={mergedContentClasses}>
      <div className="w-full max-w-4xl mx-auto">
        <h2 className="text-secondary-content font-gokhan text-center text-xl font-bold tracking-wider mb-2">
          Guest Book
        </h2>

        <form onSubmit={handleSubmit} className="flex flex-col mb-2">
          <div className="flex flex-wrap gap-2 mb-0 justify-end">
            {/* H1 */}
            <button
              type="button"
          ...
          <textarea
            ref={textareaRef}
            className="textarea textarea-bordered text-base-content bg-base-100 placeholder-base-content/40 focus:ring-base-content mt-1 h-32 w-full rounded-md text-lg focus:border-transparent focus:ring-1 focus:outline-none resize-y"
